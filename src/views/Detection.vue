    ...
    <v-row v-else>
      <v-col cols="12">
        <v-card>
          ...
          <v-card-text>
            <div v-if="detectionGroups.length === 0">
              데이터가 없습니다.
            </div>
            <div v-else>
              {{ detectionGroups.length }}개의 그룹이 탐지되었습니다.
            </div>
          </v-card-text>
        </v-card>
      </v-col>
...
export default {
  ...
  setup() {
    ...
    const loadDetectionData = async () => {
      if (!selectedExchange.value || !selectedExchangeType.value || !selectedTimeframe.value) return
      
      loading.value = true
      try {
        const response = await detectionApi.getDetectionGroups(
          selectedExchange.value,
          selectedExchangeType.value,
          selectedTimeframe.value
        )
        detectionGroups.value = response.data
      } catch (error) {
